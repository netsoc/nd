...
class LDAPClass(type):
    ...
    def __repr__(cls):
        #
        return "<class '%s.%s' (%d objects)>" %\
            (cls.__module__, cls.__name__, len(cls))


class LDAPObject(object):
    '''The base class of all LDAP-mapped classes. Each LDAP class inherits from
    LDAPObject but is an instance of LDAPClass. This class defines per-object
    methods such as reading or writing attributes, while LDAPClass defines
    per-class methods such as instantiating new objects'''
...
#    rdn_attr = None
    ...
    def get_attribute(self, name):
        '''Read an attribute from this object in the LDAP tree and
           convert the value to Python form. For the handling of
           multi-valued attributes, see ValueSet'''
        ...
        if attr.multival:
            ...
            if len(vals) == 0:
                ...
                raise AttributeError("Attribute %s seems to be multi-valued,\
                                     but isn't declared as such" % name)
            else:
    ...
    def get_all_attribute_pairs(self):
        ...
        # include all the backlink attrs as well,
        # by default they don't show up under "*"
        d = self._raw_readattrs(["*"] + Attribute.list_backlink_attrs())
        return [(name, Attribute.get_attribute(name).ldap_to_py(val))
                for name in d for val in d[name]]
    ...
    def get_all_attributes(self):
        '''Return a list of all the attributes
           this object has (not the values)'''
        d = self._raw_readattrs(["*"] + Attribute.list_backlink_attrs())
    ...
    def set_attribute(self, name, val):
        ...
        attr = Attribute.get_attribute(name)
        if attr.multival:
            raise TypeError("%s is not a single-valued attribute,\
                            it cannot be directly assigned" % name)
        else:
            n = attr.get_ldap_name()
            if n == self.rdn_attr:
    ...
    def __getattr__(self, name):
        ...
        if name == "trait_names" or "(" in name:
            # IPython causes some spurious calls to getattr,
        ...
        if type(filter) == str:
            if hasattr(cls, "default_search_attrs"):
                filter = SearchFilter.any(
                    **dict([(a, filter) for a in cls.default_search_attrs]))
            else:
...
match_exact = lambda attr, value: "(%s=%s)" %\
    (attr, ldap.filter.escape_filter_chars(value))
match_wildcard = lambda attr, value: "(%s=%s)" % \
    (attr, ldap.filter.escape_filter_chars(value).replace("\\2a", "*"))
match_substring = lambda attr, value: "(%s=*%s*)" % \
    (attr, ldap.filter.escape_filter_chars(value))
match_exact_or_substring = lambda attr, value: "(|%s%s)" % \
    (match_exact(attr, value), match_substring(attr, value))
match_wildcard_or_substring = lambda attr, value: "(|%s%s)" % \
    (match_wildcard(attr, value), match_substring(attr, value))
match_like = lambda attr, value: "(%s~=%s)" % \
    (attr, ldap.filter.escape_filter_chars(value))
...
class Attribute(object):
    '''Attributes of objects. Each attribute has:
         * a name: _ is normalised to -, so foo_bar is entered in LDAP
           as foo-bar
         * a type: this may be int, str, a LDAP class, or a type
           in square brackets.
         ...
         * a match rule: this can usually be ignored. Certain types allow
           substring search, some do not. Defaulting to substring search
           does not work since some LDAP attribute types (e.g. int) will
           return no values in that case.
         * a backlink attribute: For many-to-many relations (such as
           User.memberOf and Group.member), one of the attributes is
           kept as the canonical reference and one is labelled a "backlink".
           In the example above, member is an attribute of type [User],
           and memberOf is an attribute of type [Group] which is a backlink
           to member. So, when memberOf is updated, it will instead update
           the underlying member attribute of the appropriate group
    ...
    def __init__(self, name, type, matchrule=None, backlink=None):
        ...
        if ldapname in known:
            if known[ldapname]._desc() != attrdesc:
                lwarn("Attribute %s redefined (from %s to %s)" %
                      (name, known[ldapname]._desc(), attrdesc))
        known[ldapname] = self
    ...
    def get_filter(self, val):
        '''Return an LDAP search filter to match
           this attribute against a given value'''
        return SearchFilter.from_raw_filter(
            self.matchrule(self.get_ldap_name(), self.py_to_ldap(val)))
    ...
    def get_attribute(name):
        ...
        if n in known:
            ...
            lwarn("Undeclared attribute %s. \
                   Defaulting to assuming it's a single-valued string." % name)
...
class ValueSet(object):
    '''LDAP allows an attribute to have multiple values. This is modelled in
       Python as an attribute which is not represented by a single value but
       by a ValueSet. ValueSet objects support addition and removal of values,
       and traversal. The += and -= syntax is supported, so User("mu").memberOf
       += Group("webteam") works as expected'''
    _backlink_attrs = {}
    ...
    def _register_backlink_attr(fwd, back):
        ...
        if fwd in b:
            if b[fwd] != back:
                lwarn("Redeclared backlink for %s (was %s, now %s)"
                      % (fwd, b[fwd], back))
        b[fwd] = back
...
class SearchFilter(object):
    '''LDAP search filters. These specify a filter matching various conditions
    on an object based on the attributes of the object. See "from_raw_filter",
    "any", and "or" for how to construct them'''
    ...
    def any(*filts, **keyvals):
        '''Construct a filter to match any of the given constraints.
        Non-keyword arguments must be SearchFilters, keyword arguments
        must be of the form attribute=value. e.g: SearchFilter.any(uid="mu",
            SearchFilter.any(member=somegroup, cn="foo"))
        ...
        if len(filts) == 0:
            ...
            return SearchFilter.from_raw_filter(
                "(|" + "".join(f.filterstr for f in filts) + ")")

    @staticmethod
    def all(*filts, **keyvals):
        '''Same as SearchFilter.any, but match all constraints rather
        than just one. SearchFilter.all() with no conditions is an error'''
        filts = list(filts)
        for k, v in keyvals.iteritems():
            filts.append(Attribute.get_attribute(k).get_filter(v))
        assert filts
    ...
    def __or__(self, other):
        '''a | b for SearchFilters a,b
           is shorthand for SearchFilter.any(a,b)'''
        return SearchFilter.any(self, other)

    def __and__(self, other):
        '''a & b for SearchFilters a,b is shorthand
           for SearchFilter.all(a,b)'''
