...
def _get_samba_domain_sid():
    return LDAPObject(
        obj_dn='sambaDomainName=NETSOC,dc=netsoc,dc=tcd,dc=ie').sambaSID
...
def generate_password():
    ...
    stdout, stderr = subprocess.Popen(
        ["pwgen", "-nc"], stdout=subprocess.PIPE).communicate()
    return stdout.strip()
...
class User(NDObject):
    '''A member of Netsoc, past or present. Every member corresponds to a User
       , even the ones without active shell accounts. If a shell account exists
       for a user (even if it is disabled) user.has_account() will return True.
       For those users who have an account, their gidNumber refers to their
       PersonalGroup (see below)'''
    rdn_attr = 'uid'
    ...
    def __init__(self, uid=None, obj_dn=None):
        if uid == "root":
            NDObject.__init__(self, obj_dn=self.root_DN)
        else:
    ...
    def sendmail(self, msg, **kw):
        ...
        sm.sendmail(msg, {"To": '"%s" <%s>' % (self.cn, self.mail)}, **kw)
    ...
    def send_new_account_email(self):
        ...
        if st == "newmember":
            ...
            print "Sending account_creation email for %r to %s"\
                % (self, self.mail)
            self.sendmail("account_creation", to=self.mail, url=url)
        else:
            # just send an email
            print "Sending account_renewed email for %r to %s"\
                % (self, self.mail)
    ...
    def merge_into(self, other):
        ...
        assert other.get_state() in [
            'shell', 'renew', 'bold', 'expired', 'dead']
        assert self.is_current_member()
        assert not other.is_current_member()
        issusername = self.get_attribute("tcdnetsoc_ISS_username") or\
            other.get_attribute("tcdnetsoc_ISS_username")
        ...
        if self.cn != other.cn:
            lwarn("Names %s and %s don't match when renewing account %s"
                  % (self.cn, other.cn, other.uid))
            other.comment("Renewed by account with non-matching name %s"
                          % self.cn)
        if self.mail != other.mail:
            lwarn("Emails %s and %s don't match when renewing account %s"
                  % (self.mail, other.mail, other.uid))
    ...
    def passwd(self, new, old=None):
        '''Change the password of a user from "old" to "new". If the old
           password is not known, "old" can be omitted but changing the
           password then requires admin permissions.

           See also generate_password.'''

        # We need to do a Password Modify Extended Operation
        # to get Samba passswords to update properly and to get secure
        # hashing of the password. This requires the old password. So, if
        # we don't have it, we temporarily reset the password via directly
        # mungling userPassword, and then to a proper modify exop.
        if old is None:
    ...
    def info(self):
        ...
        def has(priv):
            if self.has_priv(priv):
                ...
                return "no " + priv
        info = "User #%s: %s (%s), %s\n" % (
            self.uidNumber, username,
            name,
            "current member" if isCurrentMember else "not current member")
        if canBind:
            if hasShellAcct:
                info += "has shell account, " +\
                    has('webspace') + "\n"
                info += "in groups: " \
                    + ", ".join(g.cn for g in self.memberOf) + "\n"
            else:
                info += "no shell account\n"
        else:
            info += "Disabled account\n"
        info += "Member of netsoc in " +\
            ", ".join(self.tcdnetsoc_membership_year) + "\n"
        return info
    ...
    def get_state(self):
        if self.has_account():
            ...
            if self.get_attribute("tcdnetsoc_saved_password")\
                    == "***newmember***":
                return "newmember"
            else:
                return "archived"
    ...
    def set_state(self, newst, newpasswd=None):
        ...
        if st == newst:
            return
        if st == "newmember" and newst not in\
                ["archived", "shell", "newmember"]:
            raise Exception("User doesn't have an account,\
                            so it can't be set to %s" % newst)
...
#            Privilege("shell").member += self
    ...
    def get_correct_state(self):
        ...
        if st in ["shell", "renew", "expired"]:
            ...
            # new members' accounts stay as
            # they are until they set up a real account
            s = "newmember"
        elif st == "archived":
            ...
            # dead accounts only get re-enabled
            # (or more likely, archived) with admin intervention
            s = "dead"
        return s
    ...
    def check(self):
        ...
        stategroups = [Privilege(x) for x in User.states
                       if x not in ["archived", "newmember"]]
        ...
        if st in ["archived", "newmember"]:
            ...
            assert self.get_personal_group().sambaSID ==\
    ...
    def create(cls, **attrs):
        '''Create a new user. Users are always created in the
           ...
           Requires that a username (uid), full name (cn) and email
           address (mail) be chosen, all other attributes will be
           given correct defaults.

           If a password is not specified (userPassword), a random one will be
           generated.

           If a uidNumber is not specified, a new one will be allocated. If a
           gidNumber is specified, it must match the uidNumber and it will be
           taken to mean that the group has already been created.

           For users who are College students, a tcdnetsoc_ISS_username should
           be specified.

           By default, newly-created accounts will be marked as members for the
           curent year. If this is not desired, specify
           "tcdnetsoc_membership_year=[]".

           Disk quotas are set to the default for each filesystem, they can be
           changed via User.quota.
    ...
    def setup_samba_account(self):
        ...
        Posix account info for reasons best known to the "designers" of the
        SMB protocol).

        Note: the password must be changed, even to the same value, after
        this is called.
    ...
    class Quota:
        ...
        def write_size(sz):
            if sz == 0:
                return "unlimited"
            sz = float(sz)
            for name, s in reversed(
                sorted(
                    User.Quota._sizes.iteritems(), key=lambda (a, b): b)):
                if sz > 0.9 * s:
                    sz /= float(s)
                    break
            return "%.1f%s" % (sz, name)
        ...
        def set(self, sz, extra_size=10,
                bytes_per_inode=10 * 1024, inode_extra_size=10):
            ...
            self._set_quota([
                ...
                int(inodelimit *
                    (1 + 0.01 * inode_extra_size))  # inode hardlimit
            ])

        def __repr__(self):
            blocksoft, blockhard, inodesoft, inodehard = self._get_quota()
            (blockused, xblocksoft, xblockhard,
             inodeused, xinodesoft, xinodehard) = self._get_usage()
...
class Group(NDObject):
    '''A group of users. Groups may contain any number of users,
       including zero'''
    rdn_attr = 'cn'
...
class PersonalGroup(Group):
    '''A PersonalGroup is a group with the same name as a user having
       only that user as a member. Its GID is the UID of the user and its
...
class Service(NDObject):
    ...
    def has_access(self, user):
        return len(list(Privilege.search(
            SearchFilter.all(tcdnetsoc_service_granted=self,
                             member=user)))) != 0
...
class IDNumber(NDObject):
    """Allocator for new ID numbers such as UID and GID.
       The next ID is stored in the allocator object, and when
       a new one is requested the field is atomically incremented
    ...
    def _setnum(self, old, new):
        ...
        self._raw_modattrs([
            (ldap.MOD_DELETE,
             'serialNumber', str(old)),
            (ldap.MOD_ADD,
             'serialNumber', str(new))])
...
class Setting(NDObject):
    """Arbitrary configuration-style key-value setting,
       stored in LDAP to be accessible from all Netsoc machines"""
    rdn_attr = 'cn'
...
Attribute('tcdnetsoc_granted_by_privilege', [Privilege],
          backlink='tcdnetsoc_service_granted')
